import pandas as pd
import numpy as np
---------- AUX: True Range mensual ----------
def monthly_true_range(close):
"""True Range sobre serie de cierres mensuales."""
prev_close = close.shift(1)
high = low = close                   # no tenemos H-L → aproximación
tr = np.maximum(high - low,
np.maximum(np.abs(high - prev_close),
np.abs(low - prev_close)))
return tr
---------- Inercia idéntica al AFL ----------
def inertia_score(monthly_close, corte=680):
"""
monthly_close: DataFrame (columnas=tickers) con precios de cierre mensuales
