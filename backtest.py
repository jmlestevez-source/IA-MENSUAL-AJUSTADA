...
def convertir_a_mensual_con_ohlc(ohlc_data):
    """
    Convierte datos diarios OHLC a mensuales - EXACTO como en el código Python que funciona
    """
    monthly_data = {}
        ...
        for year in years:
            ...
            for i, month_abbr in enumerate(months_es, 1):
                if i in year_monthly.index.get_level_values(1):
                    ...
                    year_data[month_abbr] = f"{return_value*100:.1f}%" if not np.isnan(return_value) and not np.isinf(return_value) else "-"
                else:
            ...
            if len(year_equity) > 1:
                ...
                year_data['YTD'] = f"{ytd_return*100:.1f}%" if not np.isnan(ytd_return) and not np.isinf(ytd_return) else "-"
            else:
                year_data['YTD'] = "-"
