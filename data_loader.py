...
def load_prices_from_csv_parallel(tickers, start_date, end_date, load_full_data=True):
    """Carga precios desde CSV en PARALELO - CORREGIDA"""
    prices_data = {}
    ohlc_data = {}
    
    # CORRECCIÓN: Manejar correctamente la entrada de tickers
    if isinstance(tickers, dict) and 'tickers' in tickers:
        ticker_list = tickers['tickers']
    elif isinstance(tickers, (list, tuple)):
        ticker_list = list(tickers)
    else:
        st.error(f"❌ Formato de tickers inválido: {type(tickers)}")
        return pd.DataFrame(), {}
    
    # Limpiar tickers
    ticker_list = [str(t).strip().upper() for t in ticker_list if t and str(t).strip()]
    ...
    if prices_data:
        ...
        print(f"❌ No se pudieron cargar datos para ninguno de los {len(ticker_list)} tickers")
        # Debug: verificar algunos archivos específicos
        for ticker in ticker_list[:5]:
            csv_path = f"data/{ticker}.csv"
            if os.path.exists(csv_path):
                print(f"✓ Archivo existe: {csv_path}")
            else:
...
if run_button:
    try:
        ...
        if not use_cache:
            ...
            # CORRECCIÓN CRÍTICA: Verificar que se obtengan suficientes tickers
            tickers = list(dict.fromkeys(all_tickers_data['tickers']))
            st.success(f"✅ Obtenidos {len(tickers)} tickers únicos")
            
            # Si son muy pocos, usar fallback sin validación histórica
            if len(tickers) < 100:
                st.warning(f"⚠️ Solo {len(tickers)} tickers obtenidos. Esto parece muy poco.")
                if use_historical_verification:
                    st.warning("🔄 Probando sin verificación histórica...")
                    # Intentar obtener constituyentes actuales directamente
                    ...
                    if current_constituents and len(current_constituents['tickers']) > len(tickers):
                        ...
                        st.info(f"✅ Usando {len(tickers)} constituyentes actuales como fallback")
