...
def get_current_constituents_set(index_choice):
    # Devuelve el set de símbolos actuales del índice elegido
    if index_choice == "SP500":
...
# top_n por defecto = 5
top_n = st.sidebar.slider("Número de activos", 5, 30, 5, key="top_n_slider")
commission = st.sidebar.number_input("Comisión por operación (%)", 0.0, 1.0, 0.3, key="commission_input") / 100
corte = st.sidebar.number_input("Corte de score", 0, 1000, 680, key="corte_input")
use_historical_verification = st.sidebar.checkbox("🕐 Usar verificación histórica", value=True, key="historical_verif_chk")

st.sidebar.subheader("⚙️ Opciones de Estrategia")
fixed_allocation = st.sidebar.checkbox("💰 Asignar 10% capital a cada acción", value=False, key="fixed_alloc_chk")
avoid_rebuy_unchanged = st.sidebar.checkbox("⛽ No recomprar picks que se mantienen (evitar comisiones)", value=True, key="avoid_rebuy_chk")

st.sidebar.subheader("🛡️ Filtros de Mercado")
use_roc_filter = st.sidebar.checkbox("📉 ROC 12 meses del SPY < 0", value=False, key="roc_filter_chk")
use_sma_filter = st.sidebar.checkbox("📊 Precio SPY < SMA 10 meses", value=False, key="sma_filter_chk")
use_safety_etfs = st.sidebar.checkbox("🛡️ Usar IEF/BIL cuando el filtro mande a cash", value=False, key="safety_etfs_chk")

run_button = st.sidebar.button("🏃 Ejecutar backtest", type="primary", key="run_backtest_btn")

# Limpiar
if st.session_state.backtest_completed:
    if st.sidebar.button("🗑️ Limpiar resultados", type="secondary", key="clear_results_btn"):
        st.session_state.backtest_completed = False
        st.session_state.bt_results = None
        st.session_state.picks_df = None
        st.session_state.spy_df = None
        ...
        if os.path.exists(cache_file):
            try:
                with open(cache_file, "rb") as f:
                    ...
                    if st.sidebar.checkbox("🔄 Usar resultados en caché", value=True, key="use_cache_chk"):
                        use_cache = True
...
if st.session_state.backtest_completed and st.session_state.bt_results is not None:
    ...
    # Señales actuales (vela en formación) blindadas a constituyentes actuales
    with st.expander("🔮 Señales Actuales - Vela en Formación", expanded=True):
        ...
        try:
            if prices_df is not None and not prices_df.empty:
                ...
                # BLINDAJE: limitar universo a constituyentes actuales HOY
