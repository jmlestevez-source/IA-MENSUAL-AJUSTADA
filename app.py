...
try:
    ...
    st.success("Módulo de backtest cargado correctamente")
except ImportError as e:
    BACKTEST_AVAILABLE = False
    st.warning(f"Módulo de backtest no disponible: {str(e)}")
    # Funciones dummy para evitar errores
    def run_optimized_backtest(*args, **kwargs):
        return {"error": "Función no disponible", "total_return": 0, "sharpe_ratio": 0, "max_drawdown": 0, "num_trades": 0}
...
def create_simulation_chart(df):
    ...
    try:
        ...
        st.error(f"Error creando gráfico de simulación: {str(e)}")
        return None
...
def main():
    ...
    if data is not None and not data.empty:
        ...
        try:
            ...
            st.error(f"Error en simulación de trading: {str(e)}")
        
        # Datos recientes
        st.subheader("📋 Datos Recientes")
        try:
            recent_data = data_with_indicators.tail(10)[['Date', 'Open', 'High', 'Low', 'Close', 'Volume', 'RSI', f'SMA_{sma_short}', f'SMA_{sma_long}']].round(2)
            st.dataframe(recent_data)
        except Exception as e:
            st.error(f"Error mostrando datos recientes: {str(e)}")
                    ...
                    if backtest_results and 'error' not in backtest_results:
                        ...
                        if 'equity_curve' in backtest_results and backtest_results['equity_curve']:
                            try:
                                ...
                                st.warning(f"No se pudo mostrar la curva de equity: {str(e)}")
                        else:
                            st.info("Curva de equity no disponible en los resultados.")
                ...
                with st.spinner('Calculando retornos mensuales...'):
                    ...
                    if monthly_returns and len(monthly_returns) > 0:
                        ...
                        if not returns_df.empty:
                            ...
                            st.info("No hay datos de retornos mensuales disponibles")
                    else:
                        st.info("No hay retornos mensuales calculados")
